...
class KubeflowEndToEndTest(test_utils.BaseKubeflowTest):
  ...
  def setUpClass(cls):
    ...
    super().setUpClass()  # pylint: disable=missing-super-argument
    try:
      cls._port_forwarding_process = cls._setup_mlmd_port_forward()
    except:
      if cls._port_forwarding_process is not None:
  ...
  def _setup_mlmd_port_forward(cls) -> subprocess.Popen:
    ...
    for _ in range(max_attempts):
      ...
      time.sleep(5)
      result = subprocess.run(poll_grpc_port_command, encoding='utf-8')  # pylint: disable=subprocess-run-check
